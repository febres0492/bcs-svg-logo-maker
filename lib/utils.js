...
export function createFile (fileContent = ` this just a test`){
    const fileName = U.validateFileName()
    ...
    fs.writeFile(`./examples/${fileName}`, fileContent, (err) => {
        if(err) { console.log(`Error: ${err}`) } 
            ...
            console.log(`${c('To open')} hold ${c('Alt','y')} and click this link: ${c(`./examples/${fileName}`,'y')}`);
        }
    })
}