...
export function validateFileName(fileName = 'logo', ext = 'svg'){
    ...
    while(createFile === false ) { // checing if the file exists and updating the file name
        try {
            ...
            fs.accessSync(`./output/${fileName + ext}`, fs.F_OK)
        } catch (err) { 
            // if unwanted error
...
export function createFile (obj){
    ...
    const values = { width: 600, height: 400, ...obj }
    const shapeObj = new S[obj.shape]( values ).getSvgString()

    fs.writeFile(`./output/${fileName}`, shapeObj, (err) => {
        if(err) { console.log(`Error: ${err}`) } 
        else {
            console.log(c(`Generated ${fileName}\n `))
            console.log(`File saved in: ${c('./output/','y')} folder`);
            console.log(`${c('To open in VScode')} hold ${c('Alt','y')} and click this link: ${c(`./output/${fileName}`,'y')}`);
        }
    })
}
