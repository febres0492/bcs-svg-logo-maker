...
export function validateFileName(fileName = 'logo', ext = 'svg'){
    ...
    while(createFile === false ) { // checing if the file exists and updating the file name
        try {
            ...
            if(i === 0) { console.log(c(`File ${defaultName + ext} already exists:`,'y')) }
            if(i === 1) { console.log(c(`Files ${defaultName + ext} and ${defaultName}_0${ext} already exists:`,'y')) }
            if(i > 1) { console.log(c(`File ${defaultName}_0 to _${i-1} already exists:`,'y')) }
            console.log('\n')
            createFile = true
        }
        i++
...
export function createFile (obj){
    ...
    const values = { width: 100, height: 100, ...obj }
